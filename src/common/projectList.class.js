...
export default class indexData {
  constructor(vueComponent, dataSet) {
    ...
    this.DS = dataSet
    this.Add = config.SERVERADD()+"/huihome/api/project/diaryList4"
    this.defaultData = {
    }
  }
  request(param) {
    let DateUrl =`${this.Add}?uid=${param.uid }&cate=${param.cate}&pageIndex=${param.pageIndex}&pageSize=${param.pageSize}&lastId=${param.lastId}&orderby=${param.orderby}`
