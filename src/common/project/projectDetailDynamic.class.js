...
export default class indexData {
  constructor(vueComponent, dataSet) {
    ...
    this.DS = dataSet
    this.Add = config.SERVERADD() + "/huihome/api/project/detail2/dynamic"
    this.defaultData = {}
  }
  request(param) {
    let DateUrl = `${this.Add}?uid=${param.uid}&projid=${param.projid}&asc=${param.asc}&lastId=${param.lastId}&pageIndex=${param.pageIndex}&pageSize=${param.pageSize}&step=${param.step}`
