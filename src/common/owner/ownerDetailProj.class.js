...
export default class indexData {
  constructor(vueComponent, dataSet) {
    ...
    this.DS = dataSet
    this.Add = config.SERVERADD() + "/huihome/api/owner/detail2/proj"
    this.defaultData = {}
  }
  request(param) {
    let DateUrl = `${this.Add}?uid=${param.uid }&ownerid=${param.ownerid}&step=${param.step}&asc=${param.asc}&lastId=${param.lastId}&pageIndex=${param.pageIndex}&pageSize=${param.pageSize}`
