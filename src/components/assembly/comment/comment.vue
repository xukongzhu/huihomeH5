            ...
            <div v-if="comment.subItems.length>0" class="comment-reply">
                ...
                <div v-for="(item,index1) in comment.subItems" :key="index1" class="reply-infor">
                    <div class="reply-userinfor">
                        <div class="df">
                            <img class="reply-avatar" @click="toRuleDetail(item.user.uid,item.user.roleid,item.user.merid )" :src="item.user.avatar||avatarDefault" />
                            <img v-if="item.user.proveStatus=='0'" class="huihome-v" src="../../images/zhibo_icon_vip.png">
                            <div>
                                ...
                                <p class="reply-data">{{getDateDiff(item.occurTime)}}</p>
                            </div>
                        </div>
                        <div class="comment-opera" v-if="item.datastatus=='0'">
                            <div @click="commentshow(item.id,index1,index,item.user.name)">回复·</div>
                            <div @click="likecomment(item.id,item.liked,index1)">
                                <img v-if="item.liked=='0'" src="../../images/icon_zan.png" />
                                <img v-else src="../../images/btn_zan1.png" />
                                <span v-if="item.likeCount!='0'&&parseInt(item.likeCount)<100" :class="{'c3db8':item.liked=='1'}">{{item.likeCount}}</span>
                                <span v-else-if="item.likeCount!='0'&&parseInt(item.likeCount)>=100" :class="{'c3db8':item.liked=='1'}">99+</span>
                                <span v-else>赞</span>
                            </div>
                        </div>
                    </div>
                ...
                <div @click="replyMore()" v-if="comment.subCount>3&&comment.subCount>comment.subItems.length" class="reply-more">查看更多回复<img src="../../images/btn_zhankai.png" /></div>
                <div @click="replyTake(3)" v-else-if="comment.subCount>3" class="reply-more">收起更多回复<img src="../../images/btn_shouqi.png" /></div>
            </div>
        </div>
    </div>
</template>
...
import listSubcomment from '../../../common/comment/listSubcomment.class'
...
export default {
    ...
    methods: {
                ...
                MessageBox.confirm(delconent).then(action => {
                    ...
                    DC.request(reqParamcom).then((res) => {
                        ...
                        if (res.body.respCode == "00") {
                            if (res.body.body.delMode == '1') {
                                if (index === '') {
                                    utils.vToast(res.body.respMsg, '删除评论成功')
                                    this.$emit('delComment', comindex, 'del')
                                } else {
                                    this.replyTake(this.comment.subItems.length)
                                    this.comment.subCount = parseInt(this.comment.subCount) - 1
                                    utils.vToast(res.body.respMsg, '删除回复成功')
                                }
                            } else {
                                if (index === '') {
                                    this.comment.msgtxt = res.body.body.showText
                                    this.comment.datastatus = '-1'
                                    this.$emit('delComment', comindex, 'showText')
                                    utils.vToast(res.body.respMsg, '删除评论成功')
                                } else {
                                    this.comment.subItems[index].msgtxt = res.body.body.showText
                                    this.comment.subItems[index].datastatus = '-1'
                                    utils.vToast(res.body.respMsg, '删除回复成功')
                                }
                            }
                        } else {
        ...
        //收起更多回复
        replyTake: function(subCmtCount) {
            let CI = new commentItem(this)
            let loginUser = utils.getLoginData('loginUserAuthToken');
            let reqParamcom = {
                uid: loginUser.userId || '',//loginUser.uid,
                datatype: this.type,//loginUser.serialNo,   
                cmtid: this.comment.id,
                subCmtCount: subCmtCount
            }
            Indicator.open({
                text: '加载中...',
            });
            CI.request(reqParamcom).then((res) => {
                Indicator.close();
                if (res.body.respCode == "00") {
                    this.comment.subItems = res.body.body.items[0].subItems
                } else {
                    Toast(res.body.respMsg)
                }
            }).catch((e) => {
        ...
        //查看更多回复
        replyMore: function() {
            let LSC = new listSubcomment(this)
            let loginUser = utils.getLoginData('loginUserAuthToken');
            let subcmtid = this.comment.subItems[this.comment.subItems.length - 1].id
            let reqParamcom = {
                uid: loginUser.userId || '',//loginUser.uid,
                datatype: this.type,//loginUser.serialNo,   
                cmtid: this.comment.id,
            ...
            LSC.request(reqParamcom).then((res) => {
                Indicator.close();
                if (res.body.respCode == "00") {
                    console.log(res);
                    for (let i = 0; i < res.body.body.items.length; i++) {
                        this.comment.subItems.push(res.body.body.items[i])
                    }
                } else {
                    Toast(res.body.respMsg)
                }
